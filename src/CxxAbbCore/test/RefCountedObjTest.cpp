 ...
 *
 *
 * RefCountedObjTest.cpp
 *
 * FileId      : $Id: RefCountedObjTest.cpp 20 2012-11-22 07:46:58Z prabodar $
 *
...
class TestClass : public CxxAbb::RefCounted
{
public:
	TestClass()
	{
...
class SomeOtherClass : public CxxAbb::RefCounted
{
public:
	SomeOtherClass()
	{
...
TEST(RefCountedObjTest, Constructor)
{
	...
	EXPECT_FALSE(!varTC1);
	EXPECT_EQ(1, static_cast<int>(varTC1->refCount()));
...
TEST(RefCountedObjTest, Destructor)
{
	ASSERT_EQ(0, TestClass::i_GlbCount);
	{
		...
		EXPECT_EQ(3, static_cast<int>(varTC2->refCount()));
		EXPECT_EQ(3, static_cast<int>(varTC5->refCount()));
...
TEST(RefCountedObjTest, Assignment)
{
	CxxAbb::AutoPtr<TestClass> varTC1 = new TestClass();
	CxxAbb::AutoPtr<TestClass> varTC2;
	...
	EXPECT_EQ(2, static_cast<int>(varTC1->refCount()));
	EXPECT_FALSE(varTC2.isNull());
	EXPECT_EQ(2, static_cast<int>(varTC2->refCount()));

	varTC2 = new TestClass();

	EXPECT_EQ(1u, varTC1->refCount());
	EXPECT_EQ(1u, varTC2->refCount());

	varTC1 = varTC2;

	EXPECT_EQ(2u, varTC1->refCount());
	EXPECT_EQ(2u, varTC2->refCount());
}

TEST(RefCountedObjTest, Operators)
{
	...
	EXPECT_EQ(2u, varTC4->refCount());
	EXPECT_EQ(2u, varTC5->refCount());
}

TEST(RefCountedObjTest, Casting)
{
	{
		CxxAbb::AutoPtr<TestClass> varTC1;
