 ...
 * DateTime.h
 *
 * FileId      : $Id: DateTime.h 20 2012-11-22 07:46:58Z prabodar $
 *
...
#ifndef CXXABB_CORE_DATETIME_H_
#define CXXABB_CORE_DATETIME_H_
...
/** @brief Timezone independent Date and Time representation. All times are in UTC.
 * @see Use LocalDateTime for timezone aware representation.
...
public:
    ...
    /** @brief Create DateTime form current time instance
	 */
	DateTime();

    /** @brief Create DateTime form give time instance
	 */
	DateTime(CxxAbb::Timestamp _timestamp);

    /** @brief Create DateTime form Epoch value
     * Ex: 1348722321 ~ Thu Sep 27 05:05:21 UTC 2012
	 */
	DateTime(std::time_t _tEpochSecs);

    /** @brief Create DateTime form Epoch in millisecond precision
     * Ex: 1348722321.096 ~ Thu Sep 27 05:05:21.096 UTC 2012
	 */
	DateTime(double _dEpochMsecs);

	/** @brief Create DateTime with Gregorian date times
	 */
	DateTime(short _iYear, short _iMonth, short _iDay, short _iHour = 0, short _iMinute = 0,
		short _iSecond = 0, short _iMSecond = 0, short _iMuSecond = 0);

	DateTime(Timestamp::UtcTimeVal _utcTime, Timestamp::TimeDiff _diff);

	DateTime(const DateTime & _rhs);

	~DateTime();

	void Swap(DateTime& _rhs);

	DateTime & Set(short _iYear, short _iMonth, short _iDay, short _iHour = 0,
	...
	/** @brief Timestamp of current DateTime
	 */
	...
	DateTime& operator = (const DateTime& _rhs);
	DateTime& operator = (const Timestamp& _rhs);
	DateTime& operator = (JulianDay _jd);
	...
	DateTime  operator +  (const Timespan& _span) const;
	DateTime  operator -  (const Timespan& _span) const;
	Timespan  operator -  (const DateTime& _dateTime) const;
	DateTime& operator += (const Timespan& _span);
	DateTime& operator -= (const Timespan& _span);
